...
`wa-api-rs` is a Rust client and webhook helper crate for the WhatsApp Cloud API.
...
This crate is intentionally application-neutral. It does not include persistence, identity middleware, job queues, or HTTP server routing.
...
```toml
[dependencies]
wa-api-rs = { git = "https://github.com/rust-ports/wa-api-rs", branch = "api-sdk" }
```
...
async fn main() -> wa_api_rs::Result<()> {
    let config = WhatsAppApiConfig::for_phone_number(
        "PHONE_NUMBER_ID",
        "ACCESS_TOKEN",
    );
    let client = WhatsAppApiClient::new(config)?;

    let recipient = Recipient::phone("15551234567");
    let message = TextMessage::new("Hello from Rust")?.into();

    let response = client
        .send_message(&recipient, &message, SendMessageOptions::default())
        .await?;
